-- Nieuw veld "type feest" (lentefeest, communie, verjaardag, ...) toevoegen
-- aan de reserveringsdetails en aan de maak_reservering-functie.

alter table public.reservering_details
  add column if not exists type_feest text;

-- De functie krijgt een extra parameter, dus de oude signatuur eerst weg
-- (anders ontstaat er een tweede overload naast de oude).
drop function if exists public.maak_reservering(date, text, text, text, int, text, text, int, text, text);
...
revoke execute on function public.maak_reservering(date, text, text, text, int, text, text, int, text, text, text)
  from public, anon, authenticated;
grant execute on function public.maak_reservering(date, text, text, text, int, text, text, int, text, text, text)
  to service_role;
