...
-- kort dag om het feest voor te bereiden. "Vandaag" wordt in Belgische tijd
-- bepaald (de database draait in UTC). Formulier en edge function hanteren
-- dezelfde grens; deze controle is het sluitstuk.
--
...
begin
  ...
  if p_datum < v_vandaag then
    raise exception 'Deze datum is voorbij.';
  end if;
  if p_datum < v_vandaag + 2 then
    raise exception 'Reserveren kan tot uiterlijk 2 dagen vooraf. Kies een latere datum, of bel ons voor een last-minute vraag.';
  end if;
  if v_weekdag then
